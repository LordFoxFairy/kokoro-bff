...
5. cursor 只定位位置，不授予权限。所有 lookup/replay 同时要求受信 tenant 与 path session；foreign/unknown cursor
   统一返回 `400 invalid_event_cursor`。
6. Redis 只对 hash-scoped channel 执行 best-effort `PUBLISH`。没有 AG-UI Redis key/stream，publish 失败不回滚事实，
   replay 不读取 Redis。
7. HTTP 先 drain PostgreSQL，再按持久化 source watermark 拉取 Agent、提交投影、重新从 PostgreSQL发送。已提交终态
   可在 BFF 重启和 Agent disabled/unavailable 时独立 replay。
...
- Redis notification 丢失只影响唤醒延迟，不影响历史正确性；当前实现仍用请求驱动 Agent polling，并未消费通知。
- source ingestion 尚非后台 durable consumer；首次摄取前的 Agent history 丢失仍不可恢复。
...
- 定义 retention window、safe floor、cursor-expired error 与 GC/restore tests；
- 建立后台主动 source consumer 与 Redis notification subscriber/poll fallback；
- 增加 projection lag/contention metrics、结构化日志与 SLO 告警；
- 将 Conversation/Message/Share 产品事实迁入 BFF PostgreSQL，并与公开 event 生命周期对账。
