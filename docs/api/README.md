...
- Chat/SSE 属于本仓 Chat 业务模块边界的 BFF v1 projection；`src/main.ts` 仅负责 HTTP 组合根和通用请求管线，资源路由位于 `src/http/routes/`；通用请求/响应、用例和幂等服务位于 `src/application/`，持久化与 owner client 位于 `src/infrastructure/`，契约位于 `src/contracts/`。不再创建独立 `kokoro-session` 或 `kokoro-chat` 子仓库。Chat 事件流使用 AG-UI canonical event type，详见 `docs/api/v1/agui-chat.md`。
