...
import type { AgentConnectionSetup, ChatSessionDetail, ChatSessionSummary, McpTransport, Project, ScheduledTask, Skill, Task } from "../../src/contracts/index.js"
import { queryOf, stableStringify } from "../../dist/http/request.js"
import type { RequestContext } from "../../src/domain/request-context.js"
...
export function githubSkillSource(value: unknown): GithubSkillSource | null {
  ...
  if (parsed.protocol !== "https:" || !["github.com", "www.github.com"].includes(parsed.hostname) || parsed.username || parsed.password || parsed.search || parsed.hash) return null
  const parts = parsed.pathname.split("/").filter(Boolean)
  if (parts.length !== 2) return null
  const owner = parts[0]
  const repository = parts[1]?.replace(/\.git$/u, "")
  if (owner === undefined || repository === undefined || !/^[A-Za-z0-9_.-]+$/u.test(owner) || !/^[A-Za-z0-9_.-]+$/u.test(repository)) return null
  return {
...
export function sessionScope(request: IncomingMessage, context: RequestContext): { scope: string; projectRef?: string } | { error: string } {
  const query = queryOf(request)
  const requestedScope = query.get("scope")?.trim()
  if (requestedScope !== undefined && requestedScope !== "" && requestedScope !== "direct") return { error: "scope must be direct when provided" }
...
export function mockControlReceipt(runId: string, commandId: string, body: Record<string, unknown>): Record<string, unknown> {
  ...
  return { run_id: runId, command_id: commandId, request_digest: `sha256:${requestDigest}`, status: "succeeded", replayed: false }
}
export function chatSessionDetailData(detail: ChatSessionDetail): ChatSessionDetail { return detail }
...
export function waitForSsePoll(request: IncomingMessage, response: ServerResponse, delayMs: number): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    const timer = setTimeout(() => { cleanup(); resolve(true) }, delayMs)
    const stop = (): void => { cleanup(); resolve(false) }
    const cleanup = (): void => {
      clearTimeout(timer)
