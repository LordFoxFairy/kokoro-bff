...
test("published browser-private policy is a deterministic read-only projection of runtime policy", async () => {
  ...
  assert.equal(
    createHash("sha256")
      ...
      .digest("hex"),
    "ed476b63205c0eaf59106dc618c138df6110ef6240ce2be50b417fea8ec800e4",
...
test("execution authorization stays outside the BFF generated client and browser-private relay", () => {
  ...
  assert.equal((iamSdk as Record<string, unknown>).introspectPlatformWorkloadToken, undefined)
  assert.equal((iamZod as Record<string, unknown>).zIntrospectPlatformWorkloadTokenBody, undefined)
  ...
  assert.equal((IAM_RELAY_POLICY as { routes: Record<string, unknown> }).routes["/internal/v1/platform-workload-tokens/introspect"], undefined)
})
