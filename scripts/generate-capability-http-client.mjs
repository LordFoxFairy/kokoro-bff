...
const output = path.join(root, "src/generated/capability-http")
const manifestPath = path.join(root, "contract/dependencies/capability-http.json")
const configPath = path.join(root, "openapi-ts.capability.config.ts")
const lockfilePath = path.join(root, "pnpm-lock.yaml")
const ownerCommit = "7f89a267d745cbb9870f52d6edb23dec1a3c469b"
const vendorPath = path.join(root, `contract/vendor/kokoro-capability/${ownerCommit}/capability-http.openapi.json`)
...
async function generate(directory) {
  ...
  await run(process.execPath, [cli, "--silent", "-f", configPath], { env: { ...process.env, CAPABILITY_HTTP_CLIENT_OUTPUT: directory } })
...
async function manifestFor(directory) {
  ...
  assert.equal(sha256(vendor), ownerDigest, "vendored Capability contract digest drifted")
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), "kokoro-capability-http-"))
  ...
  try {
    ...
    if (mode === "--write") {
      ...
      console.log(`WROTE Capability HTTP client (${generatedFiles.length} files)`)
      return
    }
    ...
    assert.deepEqual(JSON.parse(await readFile(manifestPath, "utf8")), expectedManifest, "Capability dependency manifest drifted")
    console.log(`PASS Capability HTTP client drift check (${generatedFiles.length} files, two byte-identical generations)`)
  } finally {
