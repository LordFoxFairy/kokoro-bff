...
const output = path.join(root, "src/generated/scheduler")
const manifestPath = path.join(root, "contract/dependencies/scheduler.json")
const configPath = path.join(root, "openapi-ts.scheduler.config.ts")
const lockfilePath = path.join(root, "pnpm-lock.yaml")
const ownerCommit = "92bf9e7e6724c591bab4b7fa27f08d694b59a67e"
const vendorPath = path.join(root, `contract/vendor/kokoro-scheduler/${ownerCommit}/openapi.yaml`)
...
async function generate(directory) {
  ...
  await run(process.execPath, [cli, "--silent", "-f", configPath], { env: { ...process.env, SCHEDULER_CLIENT_OUTPUT: directory } })
...
async function manifestFor(directory) {
  ...
  assert.equal(sha256(vendor), ownerDigest, "vendored Scheduler contract digest drifted")
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), "kokoro-scheduler-"))
  ...
  try {
    ...
    if (mode === "--write") {
      ...
      console.log(`WROTE Scheduler client (${generatedFiles.length} files)`)
      return
    }
    ...
    assert.deepEqual(JSON.parse(await readFile(manifestPath, "utf8")), expectedManifest, "Scheduler dependency manifest drifted")
    console.log(`PASS Scheduler client drift check (${generatedFiles.length} files, two byte-identical generations)`)
  } finally {
